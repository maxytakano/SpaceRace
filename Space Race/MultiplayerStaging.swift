...
class MultiplayerStaging: SKScene, SKPhysicsContactDelegate, MultiplayerNetworkingProtocol {
    ...
    var SKSpriteArray : NSMutableArray!
    var SKSpriteIndex  = 0
    /****************/
    var networkingEngine:MultiplayerNetworking!
    var originalY : CGFloat!
    var syncLocktime : UInt32 = 0
    var wait : Bool = true
    /****************/
    
    /****************/
    var updateSeconds : UInt32 = 0
...
//            self.beginRace()
    ...
    var prevTraveled = 0.0
    func timerUpdate2() {
        if playState == 0 {
            
        } else if playState == 1 {
            
            if(networkingEngine._isPlayer1) {
                if(updateSeconds < syncLocktime + 1) {
                    updateSeconds++
                    centiseconds++
                    starCounter++
                    asteroidCounter++
                    ...
                    pelletCounter++
                    laserCounter++
                    networkingEngine.sendWait(true)
                }
                else if(updateSeconds == syncLocktime + 1) {
                    networkingEngine.sendWait(false)
                    updateSeconds++
                    centiseconds++
                    starCounter++
                    asteroidCounter++
                ...
                pelletCounter++
                laserCounter++
                networkingEngine.sendSyncLock(updateSeconds)
...
//            
            ...
            if (ship.distTraveled * 100 > prevTraveled + 3000) {
                let node = SKSpriteArray[SKSpriteIndex] as SKCoordinates
                
                if(node.starX != nil) {
                    //addStar(node.starX!, starRot: node.starRot!)
                }
                if(node.energyX != nil) {
                    //addEnergy(node.energyX!, energyRot: node.energyRot!)
                }
                if(node.asteroidX != nil) {
                    addAsteroid(node.asteroidX!, asteroidRot: node.asteroidRot!)
                }
                prevTraveled = ship.distTraveled * 100
                SKSpriteIndex++
            }
...
//    }
    ...
    func addAsteroid(asteroidX : Int, asteroidRot : Int) {
        ...
        asteroid.position.x = CGFloat(asteroidX)
        asteroid.zRotation = CGFloat(asteroidRot)
        // send up mini rock
        asteroids.addObject(asteroid)
        asteroid.index = SKSpriteIndex
        self.addChild(asteroid)
    }
    ...
    func sendSKSpriteNodes() {
        ...
        for index in 1...800 {
            arr.addObject(createSKCoordinates())
        }
        setSKSpriteNodes(arr)
        networkingEngine.sendSKSpriteNodes(arr)
    }
    
    func setSKSpriteNodes(arr: NSMutableArray) {
        self.SKSpriteArray = arr
    }
    
    func createSKCoordinates() -> SKCoordinates {
        let starX = Int(getRandom(min: CGFloat(0.0), CGFloat(1.0)) * viewSize.width)
        let energyX = Int(getRandom(min: CGFloat(0.0), CGFloat(1.0)) * viewSize.width)
        let asteroidX = Int(getRandom(min: CGFloat(0.0), CGFloat(1.0)) * viewSize.width)
        let starRot = Int(getRandom(min: CGFloat(0.0), CGFloat(6.28)) * viewSize.width)
        let energyRot = Int(getRandom(min: CGFloat(0.0), CGFloat(6.28)) * viewSize.width)
        let asteroidRot = Int(getRandom(min: CGFloat(0.0), CGFloat(6.28)) * viewSize.width)
        let toAdd = SKCoordinates(starX: starX, energyX: energyX, asteroidX: asteroidX, starRot: starRot, energyRot: energyRot, asteroidRot: asteroidRot)
        return toAdd
    }
    
    func setSyncLock(time: UInt32) {
        self.syncLocktime = time
    }
    
    func processCollision(type: Collision, index: Int) {
        let node = self.SKSpriteArray[index] as SKCoordinates
        
        if(type == Collision.star) {
            node.starX = nil
            node.starRot = nil
        }
        else if(type == Collision.energy) {
            node.energyX = nil
            node.energyRot = nil
        }
        else if(type == Collision.asteroid) {
            node.asteroidX = nil
            node.asteroidRot = nil
        }
    }
